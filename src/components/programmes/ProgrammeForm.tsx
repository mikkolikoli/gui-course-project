...
export const AddWorkoutContext = createContext(
  {} as {
    workout: Workout;
    times: WorkoutTime[];
  },
...
export default function ProgrammeForm() {
  ...
  const handleProgramChange = (e: SelectChangeEvent<Number>) => {
    ...
    const programme = programmes.find(
      (programme) => programme.id === e.target.value,
    );
    if (programme) {
  ...
  const createNewProgram = () => {
    const newProgramme = new Programme(
      Date.now(),
      newProgrammeName,
      [] as unknown as [{ workout: Workout; times: WorkoutTime[] }],
    );
    ...
    const userData = getDoc(userDataRef).then((doc) => {
      ...
      if (data) {
        ...
        data.programmes
          ? data.programmes.push(newProgrammeData)
          : (data.programmes = [newProgrammeData]);
        setDoc(userDataRef, data);
  ...
  return (
    <Stack spacing={2} bgcolor={"gray"} mt={3}>
      <Stack direction="row" spacing={5}>
        <Stack>
          ...
          >
            {programmes.map((programme, i) => (
              <MenuItem key={i} value={programme.id}>
                {programme.name}
              </MenuItem>
            ))}
          </Select>
        ...
        <Stack>
          <InputLabel id="new-programme-label">
      ...
      {showCalendar && (
        <Button onClick={showPopup}>Add a workout to this programme</Button>
      )}
      {showCalendar && (
        <AddWorkoutContext.Provider
          value={null as unknown as { workout: Workout; times: WorkoutTime[] }}
        >
          <AddWorkoutDialog
            open={popupShown}
            handleClose={() => setPopupShown(false)}
            restDays={restDays}
          />
        </AddWorkoutContext.Provider>
      )}
